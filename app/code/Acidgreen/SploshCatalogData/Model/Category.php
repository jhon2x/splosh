 ...
 */
namespace Acidgreen\SploshCatalogData\Model;
...
class Category
{
    ...
    public function install(array $fixtures)
    {
        foreach ($fixtures as $fileName) {
            ...
            $this->logger->debug(__METHOD__.' :: header? '. print_r($header, true));
    ...
    protected function createCategory($row)
    {
        ...
        if (!$category) {
            $parentCategory = $this->categoryFactory->create()->load($row['path']); // $this->getCategoryByPath($row['path']);
            $data = [
                'store_id' => 0,
            ...
            $this->logger->debug('Category '.$category->getName().' saved. ID: '.$category->getId());
        }
    }
}
