<?php
namespace Acidgreen\CatalogSearch\Model\Adapter\Mysql\Filter;
...
/**
 * Preprocessor class override for \Magento\CatalogSearch\Model\Adapter\Mysql\Filter\Preprocessor
 * Meant for overriding isAddStockFilter method for backorder-related B2B purposes
...
class Preprocessor extends \Magento\CatalogSearch\Model\Adapter\Mysql\Filter\Preprocessor
{
    ...
    private function processQueryWithField(FilterInterface $filter, $isNegation, $query)
    {
        ...
        if ($filter->getField() === 'price') {
            ...
            return '`category_products_index`.' . $query;
    ...
    /**
     * Override core isAddStockFilter method 
     * (to be able to still get backorder products after using the layered navigation filters)
     * @return bool
     */
    private function isAddStockFilter()
    {
        /** SPL-441 - "bypass" this if B2B since we use Force Backorder attribute in B2B sites */
        $currentStoreCode = $this->scopeResolver->getScope()->getCode();
        if (preg_match("/b2b/", $currentStoreCode)) {
            return false;
        }
